...
package com.beditz.barcprogressview.lib;
...
public class ArcProgressStackViewLibrary extends View {
    ...
    public ArcProgressStackViewLibrary(final Context context) {
        this(context, null);
    }

    public ArcProgressStackViewLibrary(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }


    public ArcProgressStackViewLibrary(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        try {
            ...
            setStartAngle(  (int) DEFAULT_START_ANGLE

            );
            setSweepAngle(  (int) DEFAULT_SWEEP_ANGLE

            );
            setProgressModelOffset( DEFAULT_MODEL_OFFSET

            );
            setModelBgEnabled( true

            );

            // Set orientation
            final int orientationOrdinal = 0;
            ...
            if (isInEditMode()) {
                String[] preview = null;
                try {
                    final int previewId = 0;
                    preview = previewId == 0 ? null : 0;
